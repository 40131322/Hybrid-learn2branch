...
This script collects training data for machine learning models to learn branching decisions
in mixed-integer linear programming (MILP) solvers. It generates datasets by solving MILP
instances and recording branching decisions made by the SCIP solver using full strong branching.

The data collection process involves:
1. Solving multiple MILP instances with the SCIP solver
2. At each branching decision, recording the problem state and the decision made by full strong branching
3. Storing this data in a structured format for training ML models
4. Using parallelism to collect data efficiently

This approach follows the imitation learning paradigm, where an ML model learns to mimic
an expensive expert policy (full strong branching) to make similar decisions at much lower cost.
"""
import os
import argparse
import pickle
...
if __name__ == "__main__":
    ...
    The script collects datasets for four types of MILP problems:
    1. setcover: Set Covering Problem
    2. cauctions: Combinatorial Auctions
    3. facilities: Facility Location Problem
    4. indset: Independent Set Problem
    
    For each problem type, it collects training, validation, and test datasets.
    """
    ...
    train_size = 50000  # Number of training samples to collect
    valid_size = 15000  # Number of validation samples to collect
    test_size = 15000    # Number of test samples to collect
    time_limit = 3600    # Time limit per instance (in seconds)
    node_limit = 500     # Maximum nodes to process per instance
    
    # Always record all nodes (probability = 1.0)
    # If set lower, would skip some nodes to reduce dataset size
    node_record_prob = 1.0

    basedir = "data/samples"
    
    # Set up problem-specific instance directories and parameters
    # Each problem type has different instance characteristics and directories
    if args.problem == 'setcover':
        # Set Covering Problem (500 rows, 1000 columns, 0.05 density)
        ...
        # Combinatorial Auctions (100 items, 500 bids)
        ...
        # Independent Set Problem (750 nodes, degree 4)
        ...
        # Facility Location Problem (100 facilities, 100 customers, 5 facilities per customer)
            instances_train = glob.glob('data/instances/facilities/test_100_100_5/*.lp')
            instances_valid = glob.glob('data/instances/facilities/valid_100_100_5/*.lp')
            instances_test  = glob.glob('data/instances/facilities/train_100_100_5/*.lp')
    ...
    # Special case for independent set problems: collect medium validation data
    # This collects additional data for larger instances to test generalization

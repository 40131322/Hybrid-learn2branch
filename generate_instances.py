...
def generate_capacited_facility_location(random, filename, n_customers, n_facilities, ratio):
    # Change all instances of rng to random in this function
    c_x = random.rand(n_customers)  # Changed from rng.rand
    c_y = random.rand(n_customers)  # Changed from rng.rand
    
    f_x = random.rand(n_facilities)  # Changed from rng.rand
    f_y = random.rand(n_facilities)  # Changed from rng.rand
    
    demands = random.randint(5, 35+1, size=n_customers)  # Changed from rng.randint
    capacities = random.randint(10, 160+1, size=n_facilities)  # Changed from rng.randint
    fixed_costs = random.randint(100, 110+1, size=n_facilities) * np.sqrt(capacities) \
            + random.randint(90+1, size=n_facilities)  # Changed from rng.randint
    # ... and so on for any other instances
    """
